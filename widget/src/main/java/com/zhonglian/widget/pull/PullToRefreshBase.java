 ...
 *******************************************************************************/
package com.zhonglian.widget.pull;
...
import com.zhonglian.widget.pull.internal.Utils;
import com.zhonglian.widget.pull.internal.ViewCompat;
...
////		ILoadingLayout layout = mLoadingAnimationStyle.createLoadingLayout(context, mode,
...
//	}
	...
	public abstract  ILoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs);
...
//	}
	...
	protected void onPullToRefresh() {
		switch (mCurrentMode) {
			case PULL_FROM_END:
				Log.d(LOG_TAG, "onPullToRefresh end");
				mFooterLayout.pullToRefresh();
				break;
			case PULL_FROM_START:
				Log.d(LOG_TAG, "onPullToRefresh start");
	...
	protected void onRefreshing(final boolean doScroll) {
		if (mMode.showHeaderLoadingLayout()) {
			Log.d(LOG_TAG, "onRefreshing end");
			mHeaderLayout.refreshing();
		}
		if (mMode.showFooterLoadingLayout()) {
			Log.d(LOG_TAG, "onRefreshing start");
	...
	protected void onReleaseToRefresh() {
		switch (mCurrentMode) {
			case PULL_FROM_END:
				Log.d(LOG_TAG, "onReleaseToRefresh end");
				mFooterLayout.releaseToRefresh();
				break;
			case PULL_FROM_START:
				Log.d(LOG_TAG, "onReleaseToRefresh start");
	...
	protected void updateUIForMode() {
		...
		if (this == mHeaderLayout.getParent()) {
			removeView(mHeaderLayout.getView());
		}
		if (mMode.showHeaderLoadingLayout()) {
			addViewInternal(mHeaderLayout.getView(), 0, lp);
		}

		// Remove Footer, and then add Footer Loading View again if needed
		if (this == mFooterLayout.getParent()) {
			removeView(mFooterLayout.getView());
		}
		if (mMode.showFooterLoadingLayout()) {
			addViewInternal(mFooterLayout.getView(), lp);
		}
...
//		}
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			if (DEBUG) {
				Log.d(LOG_TAG, "itemDimension: " + itemDimension);
				Log.d(LOG_TAG, "newScrollValue: " + Math.abs(newScrollValue));
			}
			if (mState != State.PULL_TO_REFRESH && itemDimension >= Math.abs(newScrollValue)) {
				if (DEBUG) {
					Log.d(LOG_TAG, "pullEvent: PULL_TO_REFRESH");
				}
				setState(State.PULL_TO_REFRESH);
			} else if (mState == State.PULL_TO_REFRESH && itemDimension < Math.abs(newScrollValue)) {
				if (DEBUG) {
					Log.d(LOG_TAG, "pullEvent: RELEASE_TO_REFRESH");
				}
...
//		}
//
//		ILoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
////			switch (this) {
////				case ROTATE:
////				default:
